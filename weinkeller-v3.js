...
function loadExtendedDashboard() {
    ...
    document.getElementById('blau-count').textContent = rings.Blau
    document.getElementById('blau-value').textContent = '€' + ringWerte.Blau.toFixed(0)
    document.getElementById('gruen-count').textContent = rings.Grün
    document.getElementById('gruen-value').textContent = '€' + ringWerte.Grün.toFixed(0)
    document.getElementById('rosa-count').textContent = rings.Rosa
    document.getElementById('rosa-value').textContent = '€' + ringWerte.Rosa.toFixed(0)
    document.getElementById('kein-count').textContent = rings['Kein Ring']
    document.getElementById('kein-value').textContent = '€' + ringWerte['Kein Ring'].toFixed(0)
    
    // Highlights (Blaue)
    const blaue = aktiv.filter(w => w.ring === 'Blau').sort((a,b) => (parseFloat(b.aktueller_wert)||0) - (parseFloat(a.aktueller_wert)||0))
    document.getElementById('highlights').innerHTML = blaue.map(w => `
        <div class="highlight-item" onclick="showDetail(${w.id})">
            <div class="highlight-title">${w.produzent} ${w.weinname} ${w.jahrgang||''}</div>
            <div class="highlight-desc">${w.traube || ''} | €${parseFloat(w.aktueller_wert||0).toFixed(0)}</div>
            <div class="highlight-meta">${w.ratings || ''}</div>
        </div>
    `).join('')
    ...
    document.getElementById('drink-alerts').innerHTML = `
        <div style="margin-bottom: 1.5rem;">
            <h3 style="color: #ff1493; margin-bottom: 0.75rem;">🌸 ${rosa.length} ROSA - 2026 TRINKEN! (AM PEAK)</h3>
            ${rosa.map(w => `
                <div class="highlight-item" onclick="showDetail(${w.id})" style="border-left-color: #ff1493;">
                    <div class="highlight-title">${w.produzent} ${w.weinname} ${w.jahrgang||''}</div>
                    <div class="highlight-desc">€${parseFloat(w.aktueller_wert||0).toFixed(0)} | ${w.trinkreife_ende || ''}</div>
                </div>
            `).join('')}
        </div>
        <div>
            <h3 style="color: #32cd32; margin-bottom: 0.75rem;">🟢 ${gruen.length} GRÜN - Bis spätestens 2027</h3>
            ${gruen.slice(0,10).map(w => `
                <div class="highlight-item" onclick="showDetail(${w.id})" style="border-left-color: #32cd32;">
                    <div class="highlight-title">${w.produzent} ${w.weinname} ${w.jahrgang||''}</div>
                    <div class="highlight-desc">€${parseFloat(w.aktueller_wert||0).toFixed(0)}</div>
                </div>
            `).join('')}
            ${gruen.length > 10 ? `<div style="text-align:center;color:#666;margin-top:1rem;">...und ${gruen.length-10} weitere</div>` : ''}
        </div>
    `
}
